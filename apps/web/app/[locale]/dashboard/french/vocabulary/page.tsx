...
export default function FrenchVocabularyPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t('Vocabulary Builder')}
        description={t('Build and review your French vocabulary')}
      >
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => setShowExtract(!showExtract)}>
            <Sparkles className="w-4 h-4" />{t('Extract')}
          </Button>
          <Button variant="secondary" onClick={() => setShowAddForm(!showAddForm)}>
            <Plus className="w-4 h-4" />{t('Add Word')}
          </Button>
          {dueWords.length > 0 && (
            <Button onClick={() => { setReviewMode(true); setCurrentIndex(0); setShowAnswer(false); }}>
              <Brain className="w-4 h-4" />{t('Review')} ({dueWords.length})
            </Button>
          )}
        </div>
      </PageHeader>
      ...
      {showExtract && (
        <Card padding="md">
          <h3 className="font-semibold text-gray-900 mb-3">{t('Extract from Conversation')}</h3>
          <div className="flex gap-3">
            <Select
              ...
              className="flex-1"
            >
              <option value="">{t('Select a conversation')}</option>
            ...
            <Button onClick={() => extractMutation.mutate()} disabled={!extractConvId || extractMutation.isPending}>
              {extractMutation.isPending ? 'Extracting...' : t('Extract')}
            </Button>
          </div>
      ...
      {showAddForm && (
        <Card padding="md">
          <h3 className="font-semibold text-gray-900 mb-3">{t('Add Word')}</h3>
          <div className="grid sm:grid-cols-2 gap-3 mb-3">
            <Input placeholder={t('French word')} value={newWord} onChange={(e) => setNewWord(e.target.value)} />
            <Input placeholder={t('English translation')} value={newTranslation} onChange={(e) => setNewTranslation(e.target.value)} />
            <Input placeholder={t('Context sentence')} value={newContext} onChange={(e) => setNewContext(e.target.value)} className="sm:col-span-2" />
            <Textarea placeholder={t('Personal note')} value={newNote} onChange={(e) => setNewNote(e.target.value)} className="sm:col-span-2" rows={2} />
          </div>
          <div className="flex gap-2">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label={t('Total Words')} value={stats.total} icon={BookOpen} color="text-blue-600" bg="bg-blue-50" />
          <StatCard label={t('Mastered')} value={stats.mastered} icon={CheckCircle2} color="text-emerald-600" bg="bg-emerald-50" />
          <StatCard label={t('Due for Review')} value={stats.dueForReview} icon={AlertCircle} color="text-amber-600" bg="bg-amber-50" />
          <StatCard
            label={t('By Difficulty')}
      ...
      {reviewMode && currentWord ? (
          ...
          <div className="py-8">
            ...
            {!showAnswer ? (
              <Button className="mt-6" onClick={() => setShowAnswer(true)}>
                <RotateCcw className="w-4 h-4" />{t('Show Answer')}
              </Button>
            ) : (
              <div className="mt-6 space-y-4">
                <p className="text-xl text-blue-600 font-medium">{currentWord.translation}</p>
                <div className="flex items-center justify-center gap-4">
                  <p className="text-sm text-gray-500">{t('How well do you know this?')}</p>
                </div>
                <div className="flex items-center justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((score) => (
                    <Button
                      ...
                      className="w-16"
                    >
                      {score === 1 ? t('Forgot') : score === 5 ? t('Easy') : score}
                    </Button>
                  ))}
                </div>
              </div>
        ...
        <Card padding="lg" className="text-center">
          <div className="py-8">
            ...
            <p className="text-lg font-semibold text-gray-900">{t('All Caught Up')}</p>
            <p className="text-sm text-gray-500 mt-1">{t('No words due for review')}</p>
            <Button className="mt-4" onClick={() => setReviewMode(false)}>
              {t('Back to Vocabulary')}
            </Button>
          </div>
        </Card>
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
              {(['all', 'due', 'mastered'] as const).map((f) => (
                ...
                >
                  {f === 'all' ? t('All') : f === 'due' ? t('Due') : t('Mastered')}
                </button>
              ))}
            </div>
          ...
          {filteredWords.length === 0 ? (
            <EmptyState
              icon={BookOpen}
              title={t('No Vocabulary Words')}
              description={t('Add your first word to get started')}
              action={{ label: t('Add Word'), onClick: () => setShowAddForm(true) }}
            />
          ) : (
