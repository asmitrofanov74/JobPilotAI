...
export default function FrenchCulturalPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t('Cultural Tips')}
        description={t('Learn French culture and workplace etiquette')}
      >
          <Button variant="secondary" onClick={() => setShowHistory(!showHistory)}>
            <History className="w-4 h-4" />{showHistory ? 'New Tip' : t('History')}
        </Button>
      </PageHeader>
      ...
      {!showHistory ? (
        <>
          <Card padding="md">
            <h3 className="font-semibold text-gray-900 mb-3">{t('Get a Cultural Tip')}</h3>
            ...
            <div className="flex gap-3">
              <Input
                placeholder={t('Or type a custom topic…')}
                value={topic}
              ...
              <Button onClick={() => generateTip()} disabled={loadingTip}>
                {loadingTip ? (
                  ...
                  <><Lightbulb className="w-4 h-4" />{t('Get Tip')}</>
                )}
              </Button>
              {currentTip && (
          ...
          {currentTip ? (
            <Card padding="lg">
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 rounded-lg p-4">
                  <p className="text-sm text-gray-500 mb-1">{t('Français')}</p>
                  <p className="text-gray-900">{currentTip.tip}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-sm text-gray-500 mb-1">{t('English')}</p>
            ...
            <Card padding="lg" className="text-center">
              <div className="py-8">
                ...
                <p className="text-gray-500">{t('Generating cultural tip…')}</p>
              </div>
            </Card>
          ) : (
            <EmptyState
        ...
        <div className="space-y-4">
          ...
          {hLoading ? (
            ...
            <EmptyState
              ...
              action={{ label: t('Get Tip'), onClick: () => setShowHistory(false) }}
            />
          ) : (
