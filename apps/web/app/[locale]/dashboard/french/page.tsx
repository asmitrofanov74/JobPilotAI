...
export default function FrenchCoachPage() {
  ...
  const stats = [
    { label: t('Conversations'), value: conversations?.length ?? 0, icon: Languages, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: t('Sessions'), value: progress?.totalSessions ?? 0, icon: ScrollText, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Completed', value: progress?.completedSessions ?? 0, icon: GraduationCap, color: 'text-violet-600', bg: 'bg-violet-50' },
    { label: t('Avg Score'), value: progress?.averageScore != null ? progress.averageScore : '—', icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3 mb-6">
        {profile?.frenchVariant && (
          <Badge variant={profile.frenchVariant === 'quebec' ? 'violet' : 'blue'} className="shrink-0">
            ...
            {profile.frenchVariant === 'quebec' ? t('Quebec French') : t('France French')}
          </Badge>
        )}
        <PageHeader
          title={t('French Coach')}
          description={t('Practice French conversations tailored to your profession')}
        >
          ...
          <Button onClick={() => router.push('/dashboard/french/conversations')}>
            <MessageSquare className="w-4 h-4" />{t('New Conversation')}
          </Button>
        </PageHeader>
      </div>
      ...
      {trackerStats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label={t('Words Tracked')} value={trackerStats.total} icon={BookmarkCheck} color="text-rose-600" bg="bg-rose-50" />
          <StatCard label={t('Learned')} value={trackerStats.learned} icon={CheckCircle2} color="text-emerald-600" bg="bg-emerald-50" />
          <StatCard label={t('Difficult')} value={trackerStats.difficult} icon={AlertTriangle} color="text-amber-600" bg="bg-amber-50" />
          <StatCard label={t("Today's Words")} value={todayVocab?.words?.length ?? 0} icon={BookmarkPlus} color="text-violet-600" bg="bg-violet-50" href="/dashboard/french/vocabulary" showArrow />
        </div>
      )}
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900">{t("Today's Vocabulary")}</h3>
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Practice Scenarios')}</h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-gray-900">{t('Vocabulary Builder')}</p>
              <p className="text-sm text-gray-400 mt-0.5">{t('Build your French vocabulary')}</p>
            </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-gray-900">{t('Cultural Tips')}</p>
              <p className="text-sm text-gray-400 mt-0.5">{t('Learn French culture and etiquette')}</p>
            </div>
            <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-gray-500 ml-auto transition-colors" strokeWidth={1.5} />
          </div>
        </Link>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('Recent Conversations')}</h3>
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('Sessions by Type')}</h3>
          ...
          {sessionsData?.length === 0 ? (
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <p className="text-sm text-gray-500">{t('No sessions yet')}</p>
            </div>
