...
import { type GqlResume } from '@/lib/graphql/types';
...
export default function ResumesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('Resumes')} description={t('Manage your uploaded resumes')} />
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-medium text-gray-700">
            {uploading ? t('Uploading…') : isDragActive ? t('Drop your resume here') : t('Drag & drop or click to upload')}
          </p>
          <p className="text-xs text-gray-400">{t('PDF, DOC, or DOCX')}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={FileText} title={t('No resumes uploaded')} description={t('Upload your first resume to get started')} />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {resumes?.map((resume: GqlResume) => (
            <Card key={resume.id} hover>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-1">
                  {!resume.isPrimary && (
                    <button onClick={() => setPrimaryMutation.mutate(resume.id)} className="p-1.5 text-gray-300 hover:text-amber-500 transition-colors" title={t('Set as primary')}>
              ...
              {resume.isPrimary && <Badge variant="amber" dot>{t('Primary')}</Badge>}
              <p className="mt-3 text-xs text-gray-400">{t('Uploaded')} {formatDate(resume.createdAt)}</p>
            </Card>
          ))}
        </div>
      )}
