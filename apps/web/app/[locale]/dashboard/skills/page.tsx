...
export default function SkillsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('Skills Analysis')} description={t('Compare your profile against job requirements')} />

      <Card padding="lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">{t('Analyze New Gap')}</h2>
          <Button onClick={() => setShowForm(!showForm)} variant="secondary">
            <Zap className="w-4 h-4" />{showForm ? t('Hide Form') : t('New Analysis')}
          </Button>
        </div>
        ...
        {showForm && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('New Analysis')} value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} required />
              <Input label={t('New Analysis')} value={companyName} onChange={(e) => setCompanyName(e.target.value)} required />
            </div>
            <Textarea label={t('New Analysis')} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={4} required />
            <Textarea label={t('Your Skills')} value={userSkills} onChange={(e) => setUserSkills(e.target.value)} rows={2} placeholder="e.g. JavaScript, React, Node.js, Python" required />
            <Button onClick={() => analyzeMutation.mutate()} loading={analyzeMutation.isPending}>
              <Target className="w-4 h-4" />{t('Analyze')}
            </Button>
          </div>
        )}
      </Card>
        ...
        <Card padding="lg">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm text-gray-500">{t('Match Score')}</div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('Required Skills')}</h3>
              <div className="flex flex-wrap gap-2">
                {result.requiredSkills?.map((skill: string, i: number) => (
                  <span key={i} className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${
                    ...
                    {skill}
                    {result.missingSkills?.includes(skill) && ` ${t('Missing')}`}
                  </span>
                ))}
              </div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('Your Skills')}</h3>
              <div className="flex flex-wrap gap-2">
          ...
          {(result.recommendations?.length ?? 0) > 0 && (
            <div className="mt-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('Recommendations')}</h3>
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Previous Reports')}</h2>
        {isLoading ? (
          <LoadingState padding="sm" />
        ) : reports?.length === 0 ? (
          <EmptyState icon={TrendingUp} title={t('No Reports')} description="Run your first skill gap analysis" />
        ) : (
