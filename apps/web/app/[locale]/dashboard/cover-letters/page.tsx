...
export default function CoverLettersPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('Cover Letters')} description={t('Generate and manage cover letters')}>
        <Button onClick={() => setShowForm(!showForm)}><Sparkles className="w-4 h-4" />{t('Generate New')}</Button>
      </PageHeader>

      {showForm && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Generate Cover Letter')}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('Job Title')} value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} required />
              <Input label={t('Company Name')} value={companyName} onChange={(e) => setCompanyName(e.target.value)} required />
            </div>
            <Textarea label={t('Job Description')} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={4} required />
            <Select label={t('Tone')} value={tone} onChange={(e) => setTone(e.target.value)}>
              {TONES.map((t) => (
                <option key={t} value={t}>{capitalize(t)}</option>
              ))}
            </Select>
            <div className="flex gap-3">
              <Button type="submit" loading={generateMutation.isPending}><Sparkles className="w-4 h-4" />{t('Generate')}</Button>
          ...
          {generatedContent && (
            <div className="mt-6 p-5 bg-gray-50 rounded-lg border border-gray-100">
              <h3 className="text-sm font-semibold text-gray-900 mb-2">{t('Generated Cover Letter')}</h3>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={PenLine} title={t('No cover letters yet')} description={t('Generate your first cover letter')} action={{ label: t('Generate New'), onClick: () => setShowForm(true) }} />
      ) : (
