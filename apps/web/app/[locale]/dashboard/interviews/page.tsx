...
export default function InterviewsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('Interviews')} description={t('Track and manage your upcoming interviews')}>
        <Button onClick={() => setShowForm(!showForm)}><Plus className="w-4 h-4" />{t('Schedule')}</Button>
      </PageHeader>

      {showForm && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Schedule New Interview')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Select label={t('Job Application')} value={formData.jobApplicationId} onChange={(e) => setFormData({ ...formData, jobApplicationId: e.target.value })} required>
            ...
            <Select label={t('Type')} value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })}>
              {INTERVIEW_TYPES.map((ty) => (
                <option key={ty} value={ty}>{capitalize(ty)}</option>
              ))}
            </Select>
            <Input label={t('Round')} type="number" min={1} value={formData.round} onChange={(e) => setFormData({ ...formData, round: parseInt(e.target.value) || 1 })} />
            <Input label={t('Scheduled At')} type="datetime-local" value={formData.scheduledAt} onChange={(e) => setFormData({ ...formData, scheduledAt: e.target.value })} required />
      ...
      {isLoading ? (
        ...
        <EmptyState icon={MessageSquare} title={t('Interviews')} description={t('Track and manage your upcoming interviews')} action={{ label: t('Schedule'), onClick: () => setShowForm(true) }} />
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                   <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Type')}</th>
                   <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Round')}</th>
                   <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Scheduled At')}</th>
                   <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Location')}</th>
                  ...
                  <tr key={interview.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      {interview.isCompleted ? (
                        <Badge variant="emerald" dot><CheckCircle2 className="w-3 h-3" /> {t('Completed')}</Badge>
                      ) : (
                        <Badge variant="amber" dot>{t('Pending')}</Badge>
                      )}
                    </td>
