...
export default function DashboardPage() {
  ...
  const aiLinks = [
    { text: t('Need a cover letter?'), desc: t('Generate one from any job posting'), href: '/dashboard/cover-letters' },
    { text: t('Identify skill gaps'), desc: t('Compare your profile to job requirements'), href: '/dashboard/skills' },
    { text: t('Track your progress'), desc: t('View application analytics'), href: '/dashboard/analytics' },
  ];

  return (
    <div className="space-y-8">
      <PageHeader title={t('Dashboard')} description={t("Here's what's happening with your job search")} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <h3 className="font-semibold text-gray-900 mb-4">{t('Recent Activity')}</h3>
          <div className="flex items-start gap-3 text-sm">
            <div className="w-8 h-8 bg-gray-50 rounded-lg flex items-center justify-center shrink-0">
              <Briefcase className="w-4 h-4 text-gray-400" strokeWidth={1.5} />
            </div>
            <div>
              <p className="text-gray-700">{t('No recent applications')}</p>
              <p className="text-gray-400 text-xs mt-0.5">{t('Start by browsing jobs')}</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('AI Assistant')}</h3>
