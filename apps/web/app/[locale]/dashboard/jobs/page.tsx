...
export default function JobsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('Job Applications')} description={t('Track and manage your opportunities')}>
        {meta && meta.total > 0 && (
          <button
            onClick={() => {
        ...
        <Link href="/dashboard/jobs/new">
          <Button><Plus className="w-4 h-4" />{t('Add Job')}</Button>
        </Link>
      </PageHeader>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder={t('Search companies or positions…')}
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Briefcase} title={t('No jobs yet')} description={t('Start tracking your job applications')} action={{ label: t('Add Your First Job'), onClick: () => window.location.href = '/dashboard/jobs/new' }} />
      ) : (
            ...
            <table className="w-full">
              <thead className="sticky top-0 z-10 bg-gray-50">
                <tr className="bg-gray-50">
                  ...
                  <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Actions')}</th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {job.jobUrl ? (
                          ...
                          <Link href={`/dashboard/jobs/${job.id}`} className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">{t('View')}</Link>
                        )}
                        <button
                          onClick={() => {
          ...
          {meta && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              <span className="text-sm text-gray-500">
                Page {meta.page} {t('of')} {totalPages} ({meta.total} jobs)
              </span>
