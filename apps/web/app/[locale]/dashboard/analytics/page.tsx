...
export default function AnalyticsPage() {
  ...
  const STATS = [
    { label: t('Total Applications'), value: totalApplications, icon: Briefcase, bg: 'bg-blue-50', color: 'text-blue-600' },
    { label: t('Interviews'), value: interviews, icon: MessageSquare, bg: 'bg-purple-50', color: 'text-purple-600' },
    { label: t('Offers'), value: offers, icon: Award, bg: 'bg-emerald-50', color: 'text-emerald-600' },
    { label: t('Acceptance Rate'), value: `${acceptanceRate}%`, icon: TrendingUp, bg: 'bg-cyan-50', color: 'text-cyan-600' },
  ];

  return (
    <div className="space-y-6">
      <PageHeader title={t('Analytics')} description={t('Track your job search progress')} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Application Funnel')}</h2>
          {funnelLoading ? (
          ...
          {monthlyLoading ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyData}>
                ...
                <YAxis tick={{ fontSize: 12 }} />
                <Tooltip />
                <Line type="monotone" dataKey="applications" stroke="#3b82f6" strokeWidth={2} name={t('Total Applications')} />
                <Line type="monotone" dataKey="interviews" stroke="#8b5cf6" strokeWidth={2} name={t('Interviews')} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </Card>
