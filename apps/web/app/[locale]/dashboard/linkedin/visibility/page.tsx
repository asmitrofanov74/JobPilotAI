...
export default function VisibilityPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t('Visibility Analyzer')}</h1>
        <p className="text-gray-500 mt-1">{t('Optimize your LinkedIn profile with AI-powered analysis')}</p>
      </div>

      <Card padding="lg">
        <div className="space-y-4">
          <Input label={t('Headline')} value={headline} onChange={(e) => setHeadline(e.target.value)} placeholder={t('Your current headline')} required />
          <Textarea label={t('About Section')} value={about} onChange={(e) => setAbout(e.target.value)} rows={3} placeholder={t('Paste your About section')} required />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('Skills (comma separated)')} value={skills} onChange={(e) => setSkills(e.target.value)} placeholder={t('eg React, TypeScript, AWS')} required />
            <Input label={t('Target Roles (comma separated)')} value={targetRoles} onChange={(e) => setTargetRoles(e.target.value)} placeholder={t('eg Senior Engineer, Tech Lead')} required />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Input label={t('Target Locations (optional)')} value={targetLocations} onChange={(e) => setTargetLocations(e.target.value)} placeholder={t('eg Toronto, Remote')} />
            <Input label={t('Industry (optional)')} value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t('eg Technology')} />
            <Input label={t('Experience Level (optional)')} value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)} placeholder={t('eg Senior')} />
          </div>
          <Button onClick={() => analyzeMutation.mutate()} loading={analyzeMutation.isPending}>
            <Eye className="w-4 h-4" />{t('Analyze Visibility')}
          </Button>
        </div>
      </Card>
      ...
      {output && (
        <Card padding="lg">
          ...
          {output.recruiterAppeal && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('Recruiter Appeal')}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <BulletList title={t('Strengths')} items={output.recruiterAppeal.strengths} dotColor="bg-emerald-500" titleClassName="text-emerald-600" />
                <BulletList title={t('Improvements')} items={output.recruiterAppeal.improvements} dotColor="bg-amber-500" titleClassName="text-amber-600" />
              </div>
            </div>
          )}
          {output.competitorComparison?.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('Industry Comparison')}</h3>
