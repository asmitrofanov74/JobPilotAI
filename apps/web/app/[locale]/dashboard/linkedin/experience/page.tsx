...
export default function ExperiencePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t('Experience Optimizer')}</h1>
      ...
      <Card padding="lg">
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">{t('Experience Entries')}</h2>
          ...
          {entries.map((entry, i) => (
            <div key={i} className="p-4 bg-gray-50 rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">{t('Entry')} {i + 1}</span>
                {entries.length > 1 && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <Input label={t('Current Company')} value={entry.company} onChange={(e) => updateEntry(i, 'company', e.target.value)} placeholder={t('eg Google')} required />
                <Input label={t('Role')} value={entry.role} onChange={(e) => updateEntry(i, 'role', e.target.value)} placeholder={t('eg Senior Software Engineer')} required />
              </div>
              <Input label={t('Duration (optional)')} value={entry.duration} onChange={(e) => updateEntry(i, 'duration', e.target.value)} placeholder={t('eg Jan 2020 - Present')} />
              <Textarea label={t('Description')} value={entry.description} onChange={(e) => updateEntry(i, 'description', e.target.value)} rows={3} placeholder={t('Describe your responsibilities and impact')} required />
              <Input label={t('Key Achievements (comma separated, optional)')} value={entry.achievements} onChange={(e) => updateEntry(i, 'achievements', e.target.value)} placeholder={t('eg Reduced latency by 40%, Led team of 10')} />
            </div>
          ))}
          <div className="flex items-center gap-4">
            <Input label={t('Industry (optional)')} value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t('eg Technology')} className="max-w-xs" />
          ...
          <Button onClick={() => optimizeMutation.mutate()} loading={optimizeMutation.isPending}>
            <BarChart3 className="w-4 h-4" />{t('Optimize Experience')}
          </Button>
        </div>
      </Card>
      ...
      {output && output.optimizedEntries?.length > 0 && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('Optimized Entries')}</h2>
          <div className="space-y-6">
            {output.optimizedEntries.map((entry: { company?: string; role?: string; originalDescription?: string; improvements?: string[]; optimizedDescription?: string; keyChanges?: string[] }, i: number) => (
              <div key={i} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm text-gray-500">{entry.company}</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs font-semibold text-gray-500 mb-1">{t('Original')}</p>
                    <p className="text-sm text-gray-700">{entry.originalDescription}</p>
                  </div>
                  <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                    <p className="text-xs font-semibold text-emerald-600 mb-1">{t('Optimized')}</p>
          ...
          <BulletList title={t('Overall Tips')} items={output.overallTips} dotColor="bg-amber-500" />
        </Card>
      )}
    </div>
  );
