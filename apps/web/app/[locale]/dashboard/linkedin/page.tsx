...
export default function LinkedinProfilePage() {
  ...
  const TOOLS = [
    { label: t('Headline Generator'), href: '/dashboard/linkedin/headline', icon: Sparkles, desc: 'Create keyword-rich headlines that attract recruiters' },
    { label: t('About Section'), href: '/dashboard/linkedin/about', icon: FileText, desc: 'Write compelling professional stories' },
    { label: t('Experience Optimizer'), href: '/dashboard/linkedin/experience', icon: BarChart3, desc: 'Optimize work descriptions with impact metrics' },
    { label: t('Resume Comparison'), href: '/dashboard/linkedin/resume-comparison', icon: Search, desc: 'Align your resume with your LinkedIn profile' },
    { label: t('Visibility Analyzer'), href: '/dashboard/linkedin/visibility', icon: UserCheck, desc: 'Improve recruiter search discoverability' },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t('LinkedIn Optimizer')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card padding="md" className="border-blue-100 bg-blue-50/30">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{t('Profile Analyzer')}</h3>
              <p className="text-xs text-gray-500 mt-1">{t('Optimize your LinkedIn profile with AI-powered analysis')}</p>
            </div>
            <Badge variant="blue" className="ml-auto">{t('Active')}</Badge>
          </div>
        </Card>
        {TOOLS.map((tool) => (
      ...
      <Card padding="lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">{t('Profile Analysis')}</h2>
          <Button onClick={() => setShowForm(!showForm)} variant="secondary">
            <UserCheck className="w-4 h-4" />{showForm ? t('Hide Form') : t('New Analysis')}
          </Button>
        </div>
        {showForm && (
          <div className="space-y-4">
            <Input label={t('LinkedIn Profile URL')} value={profileUrl} onChange={(e) => setProfileUrl(e.target.value)} placeholder="https://linkedin.com/in/username" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('Current Role')} value={currentRole} onChange={(e) => setCurrentRole(e.target.value)} />
              <Input label={t('Current Company')} value={currentCompany} onChange={(e) => setCurrentCompany(e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('Industry')} value={industry} onChange={(e) => setIndustry(e.target.value)} />
              <Input label={t('Location')} value={location} onChange={(e) => setLocation(e.target.value)} />
            </div>
            <Input label={t('Headline')} value={headline} onChange={(e) => setHeadline(e.target.value)} placeholder="Current LinkedIn headline" />
            <Textarea label={t('About Section')} value={about} onChange={(e) => setAbout(e.target.value)} rows={3} placeholder="Paste your current About section" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('Experience Level')} value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)} />
              <Input label={t('Skills (comma separated)')} value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="e.g. React, TypeScript, AWS" />
            </div>
            <Button onClick={() => analyzeMutation.mutate()} loading={analyzeMutation.isPending}>
              <UserCheck className="w-4 h-4" />{t('Analyze Profile')}
            </Button>
          </div>
        )}
      </Card>
      ...
      {output && (
        <Card padding="lg">
          <ScoreDisplay score={output.overallScore} label={t('Overall Score')} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <BulletList title={t('Strengths')} items={output.strengths} dotColor="bg-emerald-500" titleClassName="text-emerald-700" />
            <BulletList title={t('Areas to Improve')} items={output.weaknesses} dotColor="bg-red-500" titleClassName="text-red-700" />
          </div>
          {output.recommendations?.length > 0 && (
            <div className="mt-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('Recommendations')}</h3>
          ...
          {output.keywordAnalysis && (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <h3 className="text-xs font-semibold text-gray-700 mb-2">{t('Top Keywords')}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {output.keywordAnalysis.topKeywords?.map((k: string, i: number) => <Badge key={i} variant="blue">{k}</Badge>)}
                </div>
              </div>
              <div>
                <h3 className="text-xs font-semibold text-gray-700 mb-2">{t('Missing Keywords')}</h3>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div className="text-center p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-xs text-gray-500">{t('Keyword Density')}</div>
              </div>
            </div>
          )}
        </Card>
