...
export default function LoginPage() {
  ...
  return (
      ...
      leftContent={
        <>
          <blockquote className="text-white/80 text-lg leading-relaxed italic">
            &ldquo;{t('This platform completely transformed how I manage my job search – the AI cover letter generator alone saved me hours')}&rdquo;
          </blockquote>
          <div className="flex items-center gap-3 mt-6">
            ...
            <div>
              <p className="text-white font-medium text-sm">{t('Alex Kim')}</p>
              <p className="text-white/50 text-xs">{t('Software Engineer at Google')}</p>
            </div>
          </div>
          <div className="flex items-center gap-6 mt-12 text-white/20 text-xs">
            <span>{t('AI Cover Letters')}</span>
            <span>{t('Skill Analysis')}</span>
    ...
    >
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-1">{t('Welcome back')}</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm">{t('Sign in to your account')}</p>
      </div>
      ...
      <Card padding="sm" className="mt-8">
        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wider">{t('Demo Credentials')}</p>
