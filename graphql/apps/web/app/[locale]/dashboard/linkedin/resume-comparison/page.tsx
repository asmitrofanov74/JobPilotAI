...
export default function ResumeComparisonPage() {
  ...
  const t = useTranslations('linkedin.resumeComparisonPage');
    ...
    <div className="space-y-6">
      ...
      <Card padding="lg">
        <div className="space-y-4">
          <Select label={t('selectResume')} value={selectedResumeId} onChange={(e) => setSelectedResumeId(e.target.value)} required>
            <option value="">{t('chooseResume')}</option>
            {resumes?.map((r: { id: string; title: string }) => <option key={r.id} value={r.id}>{r.title}</option>)}
          </Select>
          <Input label={t('linkedinHeadline')} value={linkedinHeadline} onChange={(e) => setLinkedinHeadline(e.target.value)} placeholder={t('linkedinHeadlinePlaceholder')} />
          <Textarea label={t('linkedinAbout')} value={linkedinAbout} onChange={(e) => setLinkedinAbout(e.target.value)} rows={3} placeholder={t('linkedinAboutPlaceholder')} />
          <Input label={t('linkedinSkills')} value={linkedinSkills} onChange={(e) => setLinkedinSkills(e.target.value)} placeholder={t('linkedinSkillsPlaceholder')} />
          <Textarea label={t('linkedinExperience')} value={linkedinExperience} onChange={(e) => setLinkedinExperience(e.target.value)} rows={3} placeholder={t('linkedinExperiencePlaceholder')} />
      ...
      {output && (
        <Card padding="lg">
          ...
          {output.strengths?.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-emerald-700 mb-2 flex items-center gap-2">
                <CheckCircle className="w-4 h-4" /> {t('linkedin:strengths')}
              </h3>
          ...
          {output.gaps?.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-amber-700 mb-2">{t('gapsFound')}</h3>
