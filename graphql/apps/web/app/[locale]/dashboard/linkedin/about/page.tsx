...
export default function AboutPage() {
  ...
  const t = useTranslations('linkedin.aboutPage');
  ...
  return (
    <div className="space-y-6">
      ...
      <Card padding="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('linkedin.currentRole')} value={targetRole} onChange={(e) => setTargetRole(e.target.value)} placeholder={t('targetRolePlaceholder')} required />
            <Input label={t('linkedin.industry')} value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t('industryPlaceholder')} required />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('yearsOfExperience')} value={experienceYears} onChange={(e) => setExperienceYears(e.target.value)} type="number" placeholder={t('yearsOfExperiencePlaceholder')} />
            <Input label={t('currentAbout')} value={currentAbout} onChange={(e) => setCurrentAbout(e.target.value)} placeholder={t('currentAboutPlaceholder')} />
          </div>
          <Textarea label={t('keyAchievements')} value={keyAchievements} onChange={(e) => setKeyAchievements(e.target.value)} rows={2} placeholder={t('keyAchievementsPlaceholder')} required />
      ...
      {output && output.aboutSections?.length > 0 && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('generatedAboutSections')}</h2>
          {output.bestSection && (
            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-lg mb-4">
              <div className="flex items-center justify-between mb-2">
                <Badge variant="green">{t('linkedin.recommended')}</Badge>
          ...
          <BulletList title={t('writingTips')} items={output.writingTips} dotColor="bg-purple-500" titleClassName="text-gray-700" />
        </Card>
      )}
