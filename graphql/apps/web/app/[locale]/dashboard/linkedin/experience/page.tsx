...
export default function ExperiencePage() {
  ...
  const t = useTranslations('linkedin.experiencePage');
  ...
  return (
    <div className="space-y-6">
      ...
      <Card padding="lg">
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">{t('experienceEntries')}</h2>
          ...
          {entries.map((entry, i) => (
            <div key={i} className="p-4 bg-gray-50 rounded-lg space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <Input label={tLinkedin('currentCompany')} value={entry.company} onChange={(e) => updateEntry(i, 'company', e.target.value)} placeholder={t('companyPlaceholder')} required />
                <Input label="Role" value={entry.role} onChange={(e) => updateEntry(i, 'role', e.target.value)} placeholder={t('rolePlaceholder')} required />
              </div>
              <Input label={t('duration')} value={entry.duration} onChange={(e) => updateEntry(i, 'duration', e.target.value)} placeholder={t('durationPlaceholder')} />
              <Textarea label="Description" value={entry.description} onChange={(e) => updateEntry(i, 'description', e.target.value)} rows={3} placeholder={t('descriptionPlaceholder')} required />
              <Input label={t('keyAchievements')} value={entry.achievements} onChange={(e) => updateEntry(i, 'achievements', e.target.value)} placeholder={t('keyAchievementsPlaceholder')} />
            </div>
          ))}
          <div className="flex items-center gap-4">
            <Input label="Industry (optional)" value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t('industryPlaceholder')} className="max-w-xs" />
          ...
          <Button onClick={() => optimizeMutation.mutate()} loading={optimizeMutation.isPending}>
            <BarChart3 className="w-4 h-4" />{t('optimizeExperience')}
          </Button>
        </div>
      </Card>
      ...
      {output && output.optimizedEntries?.length > 0 && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('optimizedEntries')}</h2>
          <div className="space-y-6">
              ...
              <div key={i} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs font-semibold text-gray-500 mb-1">{tLinkedin('original')}</p>
                    <p className="text-sm text-gray-700">{entry.originalDescription}</p>
                  </div>
                  <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                    <p className="text-xs font-semibold text-emerald-600 mb-1">{tLinkedin('optimized')}</p>
          ...
          <BulletList title={t('overallTips')} items={output.overallTips} dotColor="bg-amber-500" />
        </Card>
      )}
    </div>
  );
