...
export default function VisibilityPage() {
  ...
  const t = useTranslations('linkedin.visibilityPage');
    ...
    <div className="space-y-6">
      ...
      <Card padding="lg">
        <div className="space-y-4">
          <Input label={t('linkedin.headline')} value={headline} onChange={(e) => setHeadline(e.target.value)} placeholder={t('linkedinHeadlinePlaceholder')} required />
          <Textarea label={t('aboutSection')} value={about} onChange={(e) => setAbout(e.target.value)} rows={3} placeholder={t('aboutSectionPlaceholder')} required />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('linkedin.skills')} value={skills} onChange={(e) => setSkills(e.target.value)} placeholder={t('skillsPlaceholder')} required />
            <Input label={t('targetRoles')} value={targetRoles} onChange={(e) => setTargetRoles(e.target.value)} placeholder={t('targetRolesPlaceholder')} required />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Input label={t('targetLocations')} value={targetLocations} onChange={(e) => setTargetLocations(e.target.value)} placeholder={t('targetLocationsPlaceholder')} />
            <Input label="Industry (optional)" value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t('industryPlaceholder')} />
            <Input label="Experience Level (optional)" value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)} placeholder={t('experienceLevelPlaceholder')} />
          </div>
          <Button onClick={() => analyzeMutation.mutate()} loading={analyzeMutation.isPending}>
            <Eye className="w-4 h-4" />{t('analyzeVisibility')}
          </Button>
        </div>
      </Card>
      ...
      {output && (
        <Card padding="lg">
          ...
          {output.recruiterAppeal && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('recruiterAppeal')}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <BulletList title={t('linkedin.strengths')} items={output.recruiterAppeal.strengths} dotColor="bg-emerald-500" titleClassName="text-emerald-600" />
          ...
          {output.competitorComparison?.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('industryComparison')}</h3>
