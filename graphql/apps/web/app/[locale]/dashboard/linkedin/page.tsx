...
export default function LinkedinProfilePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card padding="lg">
        ...
        {showForm && (
          <div className="space-y-4">
            <Input label={t('profileUrl')} value={profileUrl} onChange={(e) => setProfileUrl(e.target.value)} placeholder="https://linkedin.com/in/username" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('currentRole')} value={currentRole} onChange={(e) => setCurrentRole(e.target.value)} />
              <Input label={t('currentCompany')} value={currentCompany} onChange={(e) => setCurrentCompany(e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Textarea label={t('aboutSectionLabel')} value={about} onChange={(e) => setAbout(e.target.value)} rows={3} placeholder="Paste your current About section" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Button onClick={() => analyzeMutation.mutate()} loading={analyzeMutation.isPending}>
              <UserCheck className="w-4 h-4" />{t('analyzeProfile')}
            </Button>
          </div>
        )}
      </Card>
      ...
      {output && (
        <Card padding="lg">
          <ScoreDisplay score={output.overallScore} label={t('overallScore')} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <BulletList title={t('areasToImprove')} items={output.weaknesses} dotColor="bg-red-500" titleClassName="text-red-700" />
          </div>
          ...
          {output.keywordAnalysis && (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <h3 className="text-xs font-semibold text-gray-700 mb-2">{t('topKeywords')}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {output.keywordAnalysis.topKeywords?.map((k: string, i: number) => <Badge key={i} variant="blue">{k}</Badge>)}
                </div>
              </div>
              <div>
                <h3 className="text-xs font-semibold text-gray-700 mb-2">{t('missingKeywords')}</h3>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div className="text-center p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-xs text-gray-500">{t('keywordDensity')}</div>
              </div>
            </div>
          )}
        </Card>
