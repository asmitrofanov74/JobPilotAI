        ...
        <div className="lg:col-span-2 space-y-6">
          <Card padding="lg">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Input label={t('targetRole')} value={form.targetRole} onChange={(e) => setForm({ ...form, targetRole: e.target.value })} placeholder="e.g. Staff Engineer" />
              <Select label={t('experienceLevel')} value={form.experienceLevel} onChange={(e) => setForm({ ...form, experienceLevel: e.target.value })}>
                {EXPERIENCE_LEVELS.map(({ value, label }) => (
                  <option key={value} value={value}>{label}</option>
                ))}
              </Select>
              <Input label={t('targetLocations')} value={form.targetLocations} onChange={(e) => setForm({ ...form, targetLocations: e.target.value })} placeholder="e.g. SF, NY, Remote" />
            </div>
            <div className="mt-6">
        ...
        <div className="space-y-6">
          <Card padding="lg">
            ...
            <div className="space-y-3">
              ...
              {me.subscription?.currentPeriodEnd && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">{t('renewalDate')}</span>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                ...
                <span className="text-xs text-gray-400">{t('memberSince')} {new Date(me.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          </Card>
          ...
          <Card padding="lg">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              ...
              <p><span className="text-gray-400">{t('userId')}</span> <span className="text-xs">{me.id}</span></p>
            </div>
          </Card>
        </div>
      </div>
