    ...
    <div className="space-y-6">
      ...
      <Card padding="lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">{t('analyzeNew')}</h2>
          <Button onClick={() => setShowForm(!showForm)} variant="secondary">
            <Zap className="w-4 h-4" />{showForm ? t('hideForm') : t('newAnalysis')}
          </Button>
        </div>
        ...
        {showForm && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('newAnalysis')} value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} required />
              <Input label={t('newAnalysis')} value={companyName} onChange={(e) => setCompanyName(e.target.value)} required />
            </div>
            <Textarea label={t('newAnalysis')} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={4} required />
            <Textarea label={t('yourSkills')} value={userSkills} onChange={(e) => setUserSkills(e.target.value)} rows={2} placeholder="e.g. JavaScript, React, Node.js, Python" required />
      ...
      {result && (
        <Card padding="lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('requiredSkills')}</h3>
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('yourSkillsLabel')}</h3>
              <div className="flex flex-wrap gap-2">
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('previousReports')}</h2>
        {isLoading ? (
          <LoadingState padding="sm" />
        ) : reports?.length === 0 ? (
          <EmptyState icon={TrendingUp} title={t('noReports')} description="Run your first skill gap analysis" />
        ) : (
