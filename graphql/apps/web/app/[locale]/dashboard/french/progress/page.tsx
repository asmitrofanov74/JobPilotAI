...
export default function FrenchProgressPage() {
  ...
  const tCoach = useTranslations('frenchCoach');
  const t = useTranslations('frenchCoach.progress');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card padding="lg">
          <h3 className="font-semibold text-gray-900 mb-4">{t('scoresOverview')}</h3>
          <div className="space-y-3">
            <ScoreBar label={t('grammar')} score={progress?.grammarAvg} color="bg-blue-500" />
            <ScoreBar label={t('vocabulary')} score={progress?.vocabularyAvg} color="bg-emerald-500" />
            <ScoreBar label={t('fluency')} score={progress?.fluencyAvg} color="bg-violet-500" />
          </div>
        ...
        <Card padding="lg">
          <h3 className="font-semibold text-gray-900 mb-4">{tCoach('sessionsByType')}</h3>
          {Object.keys(sessionsByType).length === 0 ? (
            <div className="flex flex-col items-center py-8 text-center">
              <ScrollText className="w-8 h-8 text-gray-300 mb-2" strokeWidth={1.5} />
              <p className="text-sm text-gray-500">{tCoach('noSessions')}</p>
            </div>
          ) : (
            <div className="space-y-3">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card padding="lg">
          <h3 className="font-semibold text-gray-900 mb-4">{t('scenariosPracticed')}</h3>
          {conversations?.length === 0 ? (
            <div className="flex flex-col items-center py-8 text-center">
              <MessageSquare className="w-8 h-8 text-gray-300 mb-2" strokeWidth={1.5} />
              <p className="text-sm text-gray-500">{tCoach('noConversations')}</p>
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('sessionHistory')}</h3>
            <span className="text-xs text-gray-400">{sessions.length} {t('total')}</span>
          </div>
          {sessions.length === 0 ? (
            <EmptyState
              icon={ScrollText}
              title={tCoach('noSessions')}
              description="Complete a session to see your history here"
              action={{ label: t('goToOverview'), onClick: () => window.location.href = '/dashboard/french' }}
            />
          ) : (
