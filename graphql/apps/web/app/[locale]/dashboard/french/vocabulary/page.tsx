...
export default function FrenchVocabularyPage() {
  ...
  const t = useTranslations('frenchCoach.vocabulary');
  ...
  return (
    <div className="space-y-8">
      ...
      {showExtract && (
        <Card padding="md">
          <h3 className="font-semibold text-gray-900 mb-3">{t('extractTitle')}</h3>
      ...
      {showAddForm && (
        <Card padding="md">
          <h3 className="font-semibold text-gray-900 mb-3">{t('addTitle')}</h3>
          <div className="grid sm:grid-cols-2 gap-3 mb-3">
            <Input placeholder={t('frenchWord')} value={newWord} onChange={(e) => setNewWord(e.target.value)} />
            <Input placeholder={t('englishTranslation')} value={newTranslation} onChange={(e) => setNewTranslation(e.target.value)} />
            <Input placeholder={t('contextSentence')} value={newContext} onChange={(e) => setNewContext(e.target.value)} className="sm:col-span-2" />
            <Textarea placeholder={t('personalNote')} value={newNote} onChange={(e) => setNewNote(e.target.value)} className="sm:col-span-2" rows={2} />
          </div>
          <div className="flex gap-2">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label={t('totalWords')} value={stats.total} icon={BookOpen} color="text-blue-600" bg="bg-blue-50" />
          <StatCard label={t('mastered')} value={stats.mastered} icon={CheckCircle2} color="text-emerald-600" bg="bg-emerald-50" />
          <StatCard label={t('dueForReview')} value={stats.dueForReview} icon={AlertCircle} color="text-amber-600" bg="bg-amber-50" />
          <StatCard
            label={t('byDifficulty')}
      ...
      {reviewMode && currentWord ? (
        ...
        <Card padding="lg" className="text-center">
          <div className="py-8">
            ...
            <p className="text-lg font-semibold text-gray-900">{t('allCaughtUp')}</p>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
              {(['all', 'due', 'mastered'] as const).map((f) => (
                ...
                >
                  {f === 'all' ? t('all') : f === 'due' ? t('due') : t('masteredLabel')}
                </button>
              ))}
            </div>
          ...
          {filteredWords.length === 0 ? (
            <EmptyState
              icon={BookOpen}
              title={t('noVocabWords')}
              description={t('noVocabDesc')}
              action={{ label: t('addWord'), onClick: () => setShowAddForm(true) }}
            />
          ) : (
