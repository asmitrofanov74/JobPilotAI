...
export default function FrenchCoachPage() {
  ...
  const t = useTranslations('frenchCoach');
  ...
  const stats = [
    ...
    { label: t('avgScore'), value: progress?.averageScore != null ? progress.averageScore : '—', icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3 mb-6">
        {profile?.frenchVariant && (
          <Badge variant={profile.frenchVariant === 'quebec' ? 'violet' : 'blue'} className="shrink-0">
            ...
            {profile.frenchVariant === 'quebec' ? t('settings.quebecFrench') : t('settings.franceFrench')}
          </Badge>
        )}
        <PageHeader
          title={t('title')}
      ...
      {trackerStats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label={t('wordsTracked')} value={trackerStats.total} icon={BookmarkCheck} color="text-rose-600" bg="bg-rose-50" />
          ...
          <StatCard label={t('todaysWords')} value={todayVocab?.words?.length ?? 0} icon={BookmarkPlus} color="text-violet-600" bg="bg-violet-50" href="/dashboard/french/vocabulary" showArrow />
        </div>
      )}
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900">{t('todaysVocabulary')}</h3>
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('practiceScenarios')}</h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-gray-900">{t('vocabularyBuilder')}</p>
              <p className="text-sm text-gray-400 mt-0.5">{t('vocabularyDesc')}</p>
            </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-gray-900">{t('culturalTips')}</p>
              <p className="text-sm text-gray-400 mt-0.5">{t('culturalTipsDesc')}</p>
            </div>
            <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-gray-500 ml-auto transition-colors" strokeWidth={1.5} />
          </div>
        </Link>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('sessionsByType')}</h3>
