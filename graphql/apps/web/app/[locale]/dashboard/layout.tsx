...
import {
  ...
  BarChart3, Settings, Menu, X, LogOut, ChevronRight, Search, Sparkles,
  UserCheck, Languages, Brain
} from 'lucide-react';
...
const ICON_MAP: Record<string, React.ElementType> = {
  ...
  jobs: Briefcase,
  resumes: FileText,
  coverLetters: FileEdit,
  interviews: Calendar,
  interviewCoach: Brain,
  skills: Sparkles,
  linkedin: UserCheck,
  frenchCoach: Languages,
  jobScraper: Search,
  analytics: BarChart3,
  settings: Settings,
};
...
const NAV_KEYS = [
  ...
  { key: 'jobs', href: '/dashboard/jobs' },
  { key: 'resumes', href: '/dashboard/resumes' },
  { key: 'coverLetters', href: '/dashboard/cover-letters' },
  { key: 'interviews', href: '/dashboard/interviews' },
  { key: 'interviewCoach', href: '/dashboard/interview-coach' },
  { key: 'skills', href: '/dashboard/skills' },
  { key: 'linkedin', href: '/dashboard/linkedin' },
  { key: 'frenchCoach', href: '/dashboard/french' },
  { key: 'jobScraper', href: '/dashboard/scraper' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV = NAV_KEYS.map(({ key, href }) => ({
    label: t(key as keyof typeof ICON_MAP),
    href,
    icon: ICON_MAP[key],
  }));
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 transform transition-transform duration-200 lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 lg:hidden space-y-3">
