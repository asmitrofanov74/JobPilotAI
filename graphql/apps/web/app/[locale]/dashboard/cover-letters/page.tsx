  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('title')} description={t('description')}>
        <Button onClick={() => setShowForm(!showForm)}><Sparkles className="w-4 h-4" />{t('generateNew')}</Button>
      </PageHeader>

      {showForm && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('generateTitle')}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label={t('jobTitle')} value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} required />
              <Input label={t('companyName')} value={companyName} onChange={(e) => setCompanyName(e.target.value)} required />
            </div>
            <Textarea label={t('jobDescription')} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={4} required />
          ...
          {generatedContent && (
            <div className="mt-6 p-5 bg-gray-50 rounded-lg border border-gray-100">
              <h3 className="text-sm font-semibold text-gray-900 mb-2">{t('generated')}</h3>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={PenLine} title={t('noLetters')} description={t('noLettersDesc')} action={{ label: t('generateNew'), onClick: () => setShowForm(true) }} />
      ) : (
