...
export default function ScraperPage() {
  const t = useTranslations('scraper');
    ...
    <div className="space-y-6">
      ...
      <Card padding="md" className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Select label={t('postedDate')} value={postedWithin} onChange={(e) => setPostedWithin(e.target.value)}>
      ...
      {!scrape.isPending && jobs.length === 0 && scrape.data && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700">
          {t('noJobs')}. Try different keywords or location, or check <code className="bg-amber-100 px-1 rounded">SCRAPER_PROVIDERS</code> in <code className="bg-amber-100 px-1 rounded">apps/api/.env</code>.
        </div>
      )}
      ...
      {!scrape.isPending && jobs.length === 0 && !scrape.data && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('searchForJobs')}</h3>
