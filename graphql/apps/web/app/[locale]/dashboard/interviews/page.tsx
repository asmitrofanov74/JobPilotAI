...
export default function InterviewsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <Card padding="lg">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('scheduleNew')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Select label={t('jobApplication')} value={formData.jobApplicationId} onChange={(e) => setFormData({ ...formData, jobApplicationId: e.target.value })} required>
            ...
            <Input label={t('scheduledAt')} type="datetime-local" value={formData.scheduledAt} onChange={(e) => setFormData({ ...formData, scheduledAt: e.target.value })} required />
      ...
      {isLoading ? (
        ...
        <EmptyState icon={MessageSquare} title={t('title')} description={t('description')} action={{ label: t('schedule'), onClick: () => setShowForm(true) }} />
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('scheduledAt')}</th>
