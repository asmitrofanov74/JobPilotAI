...
export default function DashboardPage() {
  ...
  const stats = [
    { label: t('stats.jobApplications'), value: jobs?.paginatedItems?.length ?? jobs?.edges?.length ?? 0, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50', href: '/dashboard/jobs' },
    { label: t('stats.resumes'), value: resumes?.length ?? 0, icon: FileText, color: 'text-emerald-600', bg: 'bg-emerald-50', href: '/dashboard/resumes' },
    { label: t('stats.coverLetters'), value: coverLetters?.length ?? 0, icon: FileEdit, color: 'text-violet-600', bg: 'bg-violet-50', href: '/dashboard/cover-letters' },
    { label: t('stats.interviewQuestions'), value: questions?.length ?? 0, icon: MessageSquare, color: 'text-amber-600', bg: 'bg-amber-50', href: '/dashboard/skills' },
  ];

  const actions = [
    { label: t('actions.browseJobs.label'), desc: t('actions.browseJobs.desc'), href: '/dashboard/jobs', icon: Briefcase },
    { label: t('actions.uploadResume.label'), desc: t('actions.uploadResume.desc'), href: '/dashboard/resumes', icon: FileText },
    { label: t('actions.generateCoverLetter.label'), desc: t('actions.generateCoverLetter.desc'), href: '/dashboard/cover-letters', icon: FileEdit },
    { label: t('actions.practiceInterview.label'), desc: t('actions.practiceInterview.desc'), href: '/dashboard/skills', icon: MessageSquare },
  ];

  const aiLinks = [
    { text: t('aiLinks.coverLetter.text'), desc: t('aiLinks.coverLetter.desc'), href: '/dashboard/cover-letters' },
    { text: t('aiLinks.skillGaps.text'), desc: t('aiLinks.skillGaps.desc'), href: '/dashboard/skills' },
    { text: t('aiLinks.trackProgress.text'), desc: t('aiLinks.trackProgress.desc'), href: '/dashboard/analytics' },
  ];
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="flex items-start gap-3 text-sm">
            ...
            <div>
              <p className="text-gray-700">{t('noRecentApplications')}</p>
              <p className="text-gray-400 text-xs mt-0.5">{t('startByBrowsingJobs')}</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t('aiAssistant')}</h3>
